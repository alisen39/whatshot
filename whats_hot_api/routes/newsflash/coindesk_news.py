...
ROUTE_NAME = "coindesk-news"
SOURCE_LINK = "https://www.coindesk.com/"
FEED_URL = "https://www.coindesk.com/arc/outboundfeeds/rss"

ROUTE_META: dict = {
    "name": ROUTE_NAME,
    "title": "CoinDesk",
    "description": "CoinDesk 加密货币、区块链与数字资产最新报道",
    "link": SOURCE_LINK,
}
...
def _parse_feed(xml: str, *, limit: int = 30) -> list[NewsFlashItem]:
    ...
    if not nodes:
        raise RuntimeError("CoinDesk response is not a non-empty RSS feed")
    ...
    for node in nodes:
        ...
        media = node.find("media:content")
        image_url = _absolute_url(media.get("url") if media else None)
