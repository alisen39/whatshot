    ...
    return Request(
        {
            ...
            "method": "GET",
            "path": "/arxiv-cs-lg",
            "query_string": b"type=cs-lg",
            "headers": [],
        }
    )


async def test_arxiv_cs_lg_uses_official_atom_query(monkeypatch) -> None:
    ...
    monkeypatch.setattr(arxiv_cs_lg, "get", fake_get)
    result = await arxiv_cs_lg.handle_route(_request(), no_cache=True)

    assert captured == {
        "url": arxiv_cs_lg.FEED_URL,
        "no_cache": True,
        "response_type": "text",
    }
...
async def test_arxiv_cs_lg_rejects_empty_feed(monkeypatch) -> None:
    async def fake_get(**kwargs):
    ...
    with pytest.raises(RuntimeError, match="non-empty Atom feed"):
        await arxiv_cs_lg.handle_route(_request(), no_cache=True)
