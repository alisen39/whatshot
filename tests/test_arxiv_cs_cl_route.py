...
def _fetch_service() -> FetchService:
    route = SimpleNamespace(
        handle_route=arxiv_cs_cl.handle_route,
        category="hotlist",
        category_label="热榜",
        metadata=arxiv_cs_cl.ROUTE_META,
        validate_type=True,
    )
    return FetchService(RouteCatalog({arxiv_cs_cl.ROUTE_NAME: route}))


async def test_arxiv_cs_cl_type_reaches_route_feed(monkeypatch) -> None:
    ...
    monkeypatch.setattr(arxiv_cs_cl, "_get_list", fake_get_list)

    result = await _fetch_service().fetch(
        FetchRequest(site="arxiv-cs-cl", path_type="cs-cl")
    )

    assert observed == {"no_cache": False}
    assert result.data.name == "arxiv-cs-cl"


async def test_arxiv_cs_cl_rejects_undeclared_type() -> None:
    with pytest.raises(FetchTypeNotFoundError, match="Unknown type 'hot'"):
        await _fetch_service().fetch(
            FetchRequest(site="arxiv-cs-cl", path_type="hot")
        )
...
async def test_arxiv_cs_cl_uses_official_atom_query(monkeypatch) -> None:
...
async def test_arxiv_cs_cl_rejects_empty_feed(monkeypatch) -> None:
    async def fake_get(**kwargs):
