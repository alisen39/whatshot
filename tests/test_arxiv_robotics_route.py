    ...
    return Request(
        {
            ...
            "method": "GET",
            "path": "/arxiv-robotics",
            "query_string": b"type=cs-ro",
            "headers": [],
        }
    )


async def test_arxiv_robotics_uses_official_atom_query(monkeypatch) -> None:
    ...
    monkeypatch.setattr(arxiv_robotics, "get", fake_get)
    result = await arxiv_robotics.handle_route(_request(), no_cache=True)

    assert captured == {
        "url": arxiv_robotics.FEED_URL,
        "no_cache": True,
        "response_type": "text",
    }
...
async def test_arxiv_robotics_rejects_empty_feed(monkeypatch) -> None:
    async def fake_get(**kwargs):
    ...
    with pytest.raises(RuntimeError, match="non-empty Atom feed"):
        await arxiv_robotics.handle_route(_request(), no_cache=True)
