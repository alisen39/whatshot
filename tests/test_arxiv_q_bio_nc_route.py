    ...
    return Request(
        {
            ...
            "method": "GET",
            "path": "/arxiv-q-bio-nc",
            "query_string": b"type=q-bio-nc",
            "headers": [],
        }
    )


async def test_arxiv_q_bio_nc_uses_official_atom_query(monkeypatch) -> None:
    ...
    monkeypatch.setattr(arxiv_q_bio_nc, "get", fake_get)
    result = await arxiv_q_bio_nc.handle_route(_request(), no_cache=True)

    assert captured == {
        "url": arxiv_q_bio_nc.FEED_URL,
        "no_cache": True,
        "response_type": "text",
    }
...
async def test_arxiv_q_bio_nc_rejects_empty_feed(monkeypatch) -> None:
    async def fake_get(**kwargs):
    ...
    with pytest.raises(RuntimeError, match="non-empty Atom feed"):
        await arxiv_q_bio_nc.handle_route(_request(), no_cache=True)
