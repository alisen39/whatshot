    ...
    return Request(
        {
            ...
            "method": "GET",
            "path": "/arxiv-eess-sy",
            "query_string": b"type=eess-sy",
            "headers": [],
        }
    )


async def test_arxiv_eess_sy_uses_official_atom_query(monkeypatch) -> None:
    ...
    monkeypatch.setattr(arxiv_eess_sy, "get", fake_get)
    result = await arxiv_eess_sy.handle_route(_request(), no_cache=True)

    assert captured == {
        "url": arxiv_eess_sy.FEED_URL,
        "no_cache": True,
        "response_type": "text",
    }
...
async def test_arxiv_eess_sy_rejects_empty_feed(monkeypatch) -> None:
    async def fake_get(**kwargs):
    ...
    with pytest.raises(RuntimeError, match="non-empty Atom feed"):
        await arxiv_eess_sy.handle_route(_request(), no_cache=True)
